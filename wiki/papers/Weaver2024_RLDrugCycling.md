...
- **Fitness landscape**: empirical *E. coli* β-lactamase landscapes from Mira et al. 2015; 16 genotypes (4 binary resistance alleles: A42G, E104K, M182T, G238S in TEM-1 β-lactamase); 15 β-lactam antibiotics; fitness measured as growth rate in drug
...
- **Noise model**: Gaussian noise on fitness observations with $\sigma = \sigma_\text{modifier} \times \sigma_\text{landscape}$ where $\sigma_\text{landscape}$ is derived from experimental measurement variability in the Mira et al. data; tested at $\sigma_\text{modifier} \in \{0, 5, 10, 20, 40\}$
